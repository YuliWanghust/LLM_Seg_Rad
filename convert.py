...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb  8 09:21:40 2024
...
def load_patient(dicom_files):
    patient = {}
    for n in range(len(dicom_files)):
        dp = dparser(dicom_files[n])
...
def np_struct_from_patient(patient, struct_name, case_sensitive):
    ...
    structure_matrix = np.zeros(matrix_shape, dtype=np.float32)
    for imagenum in range(0, len(patient['images'])): 
        ...
        if 'structures' in patient:
            for id, structure in patient['structures'].items():
                ...
                if found_struct:
                    struct_slice = struct_slice_np(structure['name'], np_im_bw, structurepixlut,
                                                   structure, z, prone, feetfirst)
                    struct_slice = np.asarray(struct_slice, dtype=np.float32)
...
def struct_to_mask(dicom_files, struct_name, case_sensitive=True):
