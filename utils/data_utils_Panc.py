...
from monai.transforms import MapTransform, AsDiscreted, AddChanneld, EnsureTyped, Lambdad
...
class AddChannelIfMissingd(MapTransform):
    ...
    def __call__(self, data):
        ...
        for k in self.keys:
            ...
            if isinstance(x, torch.Tensor):
                ...
                # to tensor then add channel if needed
                x = torch.as_tensor(x)
                if x.ndim == 3:
                    x = x.unsqueeze(0)
            d[k] = x
...
def get_loader(args):
    ...
    train_transform = transforms.Compose(
        [
            ...
            AddChannelIfMissingd(keys=["label"]),
            EnsureTyped(keys=["image", "label", "text_feature"]),
            transforms.Resized(keys=["image", "label"], 
                               spatial_size=[args.roi_x, args.roi_y, args.roi_z],
                               mode=("trilinear", "nearest"),
                               align_corners=(True, None),
                               ),
            # make sure label is integer BEFORE one-hot
            Lambdad(keys="label", func=lambda x: (x.long() if isinstance(x, torch.Tensor) else torch.as_tensor(x, dtype=torch.long))),
            AsDiscreted(keys="label", to_onehot=9),          # -> [9, D, H, W] (0..8)
